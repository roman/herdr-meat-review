...
That is the whole setup. `herdr-tuicr` is not loaded with the rest of herdr.el
...
```elisp
(setq herdr-tuicr-program "/path/to/herdr-tuicr/bin/herdr-review")
```
...
| | |
| --- | --- |
| `M-x herdr-tuicr-reviews` | the panel on its own |
| `M-x herdr-tuicr-open` | review the workspace at point, or the one you are in |
| `M-x herdr-tuicr-close` | end that workspace's review |
| `+` and `-` | the same two, in the panel |
| `RET` | open the review's terminal here |

`herdr-tuicr-open` works from a row in any herdr panel, not just the Reviews
...
`herdr-tuicr-panel`. Against an older herdr.el, `(require 'herdr-tuicr)` fails
